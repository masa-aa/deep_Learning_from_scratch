...
def my_net():
    ...
    # 2
    model.add(Conv2D(32, 3, activation='relu', bias_initializer='ones'))
    model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
    model.add(BatchNormalization())

    # 3-5
    model.add(Conv2D(8, 3, bias_initializer='zeros'))
    model.add(Conv2D(64, 3, bias_initializer='ones'))
...
history = model.fit(x=x_train, y=y_train, epochs=50, batch_size=128, shuffle=True, validation_split=0.25, callbacks=[early_stopping])
